...
app.use(express.static("public",{setHeaders:(res,path)=>{if(path.endsWith(".html")||path.endsWith(".js"))res.setHeader("Cache-Control","no-store, no-cache, must-revalidate, proxy-revalidate")}}));app.get("/health",(_,res)=>res.json({ok:true,players:state.seats.filter(Boolean).length,phase:state.phase}));
