...
function lightning(){const count=crypto.randomInt(1,6),selected=[];while(selected.length<count){const c=card();if(!selected.some(x=>x.rank===c.rank&&x.suit===c.suit))selected.push({...c,multiplier:LIGHTNING_VALUES[crypto.randomInt(LIGHTNING_VALUES.length)]})}return selected}
...
module.exports={BUY_IN,TARGET,MAX_SEATS,BET_SECONDS,RESULT_SECONDS,newState,deal,lightning,multiplierFor,publicState,total};
